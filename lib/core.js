(function() {
  ...
  this._primary_record_key_from_hint = function(me, id_hint) {
    return this._primary_record_key_from_id_triplet(me, this._id_triplet_from_hint(me, id_hint));
  };

  this._primary_record_key_from_id_triplet = function() {
    var P, arity, me, pkn, pkv, pkvx, type;
    ...
    type = P[0], pkn = P[1], pkv = P[2];
    pkvx = this.escape_key_value_crumb(me, pkv);
    return "" + type + "/" + pkn + ":" + pkvx;
  };

  this._secondary_record_key_from_id_triplet = function() {
    var P, arity, me, skn, skv, skvx, type;
    me = arguments[0], P = 2 <= arguments.length ? __slice.call(arguments, 1) : [];
    ...
    type = P[0], skn = P[1], skv = P[2];
    skvx = this.escape_key_value_crumb(me, skv);
    return "" + type + "/" + skn + ":" + skvx + "/~prk";
  };

  this._id_triplet_from_hint = function(me, id_hint) {
    ...
    * using a type / PKV pair:
    ...
    switch (type_of_hint = TYPES.type_of(id_hint)) {
      case 'list':
        ...
        pkn = this._primary_key_name_from_type(me, type);
        return [type, pkn, id];
      case 'text':

        /* When the hint is a text, it is understood as a Primary or Secondary Record Key: */
        if ((R = this.split_record_key(me, id_hint, null)) != null) {
          return R;
        }
  ...
  this.split_record_key = function(me, rk, fallback) {
    var R;
    R = this.split_primary_record_key(me, rk, null);
    if (R == null) {
      R = this.split_secondary_record_key(me, rk, null);
    }
    if (R == null) {
      if (fallback !== void 0) {
        return fallback;
